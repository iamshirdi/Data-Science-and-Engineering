...
def getListForHistogramAndBoxPlot(df,spark):
    ...
    temperature=spark.sql("SELECT temperature from washing where temperature is not null")
    temperaturelist=temperature.rdd.map(lambda row:row.temperature).collect() #extracting rdd to local machine collect
...
def getListsForRunChart(df,spark):
    df.createOrReplaceTempView("washing")
    temperaturets=spark.sql("SELECT temperature,ts from washing where temperature is not null order by ts asc")
    rddts=temperaturets.rdd.sample(False,0.1).map(lambda row:(row.ts,row.temperature)) #extract rows
    ts=rddts.map(lambda (ts,voltage):ts).collect()
    temperature=rddts.map(lambda (ts,voltage):voltage).collect()
    result=ts,temperature
    return result
