...
def sdTemperature(df,spark):
    ...
    a=spark.sql("SELECT temperature from washing")
    b=a.rdd
    rddx = b.map(lambda (x) : x["temperature"]) 
    rddx=rddx.filter(bool) # removes none otherwise error
...
def skewTemperature(df,spark):    
    ...
    sdx=sqrt(rddx.map(lambda (x):pow(x-mx,2)).sum()/float(rddx.count()))
    n=float(rddx.count())
    skew=n*(rddx.map(lambda (x):pow(x-mx,3)/pow(sdx,3)).sum())/(float(n-1)*float(n-2))
    return skew 
...
def kurtosisTemperature(df,spark):  
    ...
    mx=rddx.sum()/float(rddx.count())
    sdx=sqrt(rddx.map(lambda (x):pow(x-mx,2)).sum()/float(rddx.count()))
    kurtosistemp=rddx.map(lambda (x):pow(x-mx,4)).sum()/(pow(sdx,4)*(float(rddx.count())))    
    return kurtosistemp
...
def correlationTemperatureHardness(df,spark):
    df.createOrReplaceTempView("washing")
    from math import sqrt
    a1=spark.sql("SELECT temperature from washing")
    a2=spark.sql("SELECT hardness from washing")

    b1=a1.rdd
    b2=a2.rdd
    rddx = b1.map(lambda x : x["temperature"]) 
    rddy = b2.map(lambda x : x["hardness"]) 

    rddx=rddx.filter(bool) # removes none otherwise error
    rddy=rddy.filter(bool) # removes none otherwise error
    
    
    my=rddy.sum()/float(rddy.count())
    sdy=sqrt(rddy.map(lambda (x):pow(x-my,2)).sum()/float(rddy.count()))
...
df.createOrReplaceTempView("washing")
spark.sql("select min(temperature) from washing").first()


# In[10]:


df.select('temperature').show(3)
...
spark.sql("SELECT mean(temperature) as meantemp from washing")
